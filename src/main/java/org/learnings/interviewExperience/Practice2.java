...
public class Practice2 {
    ...
    public static void findExpensiveProductUnderCategory() {
        Map<String, Optional<Product>> expensiveProdctsForCategory = products.stream()
                .collect(Collectors.groupingBy(Product::category,
                        Collectors.maxBy(Comparator.comparingDouble(Product::price))));
        expensiveProdctsForCategory.forEach((key, value) -> {
    ...
    public static void findAveragePriceOfOrderGroupedByDates() {
        orders.stream().collect(
                Collectors.groupingBy((order) -> order.date()));
    }

    /**
     * 
     * Problem 1: Find all full-time employees earning more than $75,000 and sort
     * them by hire date.
     */
     ...
     */
    private static void groupEmployessByDepartment() {
    ...
    /**
     * Problem 3: Calculate the average salary of all employees in the "Engineering"
     * department.
     * 
     */
    ...
    private static void findFirstEmployeeHiredAfter2020() {
        Optional<Employee> employee = employees.stream()
                .filter(emp -> emp.hireDate().after(new GregorianCalendar(2020, Calendar.DECEMBER, 31).getTime()))
                .findFirst();
        ...
        employees.stream()
                ...
                .collect(Collectors
                        .partitioningBy((emp) -> emp.hireDate()
                                .before(new GregorianCalendar(2020, Calendar.JANUARY, 1).getTime())))
                .forEach((k, v) -> {
                    System.out.println(k.equals(true) ? "Before 2020"
